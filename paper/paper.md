...
- `{series_uid}.tar`: A tar file that contains every instance DICOM P10 file for a given series.
- `{series_uid}/metadata.json`: A JSON file that contains DICOM tags for each instance along with additional metadata.
...
- `{series_uid}/thumbnail.{mp4|jpg}`: An (optional) small lossy thumbnail containing each frame in the series with a default size of 128x128 pixels.
